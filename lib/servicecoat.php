...
define("DIR_COAT_RESOURCES", __DIR__ . "/coat-resources/");

// #[\AllowDynamicProperties]
class ServiceCoat {
    private $root = __DIR__;
    private $generatedImageData = null;
    private $generatedSignatureData = null;
    private $signatureCrop = null;
	...
	public function prepare()
	{
		...
		//Other Variables
			...
			$this->generatedSignatureData = null;
            $this->signatureCrop = null;
	}
	
	/**
	 * Generates a WWII service coat image for a soldier
	 * 
	 * @param string $last_name - Soldier's surname to display on name tag
	 * @param string $rank_abbr - Rank abbreviation (e.g., 'Pvt.', 'Cpl.', 'Sgt.', '2Lt.')
	 * @param string $unit_key - Unit identifier (e.g., '29th')
	 * @param array $awards_abbr - Array of award abbreviations (e.g., ['eib', 'm:sharps:rifle', 'dsc'])
	 * @param int $balance - Financial balance for determining certain decorations
	 */
	public function update($last_name, $rank_abbr, $unit_key, $awards_abbr = array(), $balance = 0) {
	    if(file_exists(DIR_COAT_RESOURCES)) {
            $rank = str_replace( '/', '', str_replace('.', '', $rank_abbr) );
            $unit = $unit_key; //'29th'
            
            $this->update_servicecoatC($last_name, $rank, $unit, $awards_abbr, $balance);
	...
	// find_GD() method removed as it's no longer needed without database interaction
	// Previously used to filter awards based on General Discharge dates
...
//			if(in_array('mp',$this->scBRBadges)) $scBRMPBadge = true;
	...
	public function compileJacket()
	{
		...
		//Positioning, Alpha, Crop, & Output
			...
			ob_start();
			imagepng($this->scImage);
			$this->generatedImageData = ob_get_clean();
			ob_start();
			imagepng($this->scImageSig);
			$this->generatedSignatureData = ob_get_clean();
	...
	public function update_servicecoatC($name=0, $rank='pvt', $unit='29th', $awards=array(), $balance=0)
	{
	...
	public function getSignatureCropParameters()
	{
		return $this->signatureCrop;
	}
}
